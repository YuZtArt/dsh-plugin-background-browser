...
test('DSH tools drive a real background browser, isolate sessions and clean up', { timeout: 120000 }, async () => {
  ...
  try {
    ...
    await ui.getByRole('button', { name: '更多选项' }).click()
    ...
    await ui.getByLabel('网址', { exact: true }).fill(url + '/form')
    await uiAction(() => ui.getByLabel('网址', { exact: true }).press('Enter'))
    await ui.getByRole('img', { name: '网页预览：Background browser test' }).waitFor()
    await uiAction(() => ui.getByRole('button', { name: '后退', exact: true }).click())
    await ui.getByRole('img', { name: '网页预览：Signed in', exact: true }).waitFor()
    await uiAction(() => ui.getByRole('button', { name: '前进', exact: true }).click())
    await ui.getByRole('img', { name: '网页预览：Background browser test' }).waitFor()
    await uiAction(() => ui.getByRole('button', { name: '重新加载网页', exact: true }).click())
    await uiAction(() => ui.getByRole('button', { name: '后退', exact: true }).click())
    await ui.getByRole('img', { name: '网页预览：Signed in', exact: true }).waitFor()
    await uiAction(() => ui.getByRole('button', { name: '新建标签页', exact: true }).click())
    await ui.getByText('开始浏览', { exact: true }).waitFor()
    await ui.screenshot({ path: fileURLToPath(new URL('../.dsh/browser-empty.png', import.meta.url)) })
    await ui.setViewportSize({ width: 320, height: 760 })
    assert.ok(await ui.evaluate(() => document.querySelector('.dsh-browser').scrollWidth <= 320), 'narrow panel must not overflow')
    await ui.setViewportSize({ width: 460, height: 760 })
